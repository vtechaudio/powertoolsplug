## 2.22.4
- About Us "Brands we carry" wording made explicit about the brand relationship. Because the live catalogue stocks more brands than the four named examples (e.g. the Saws page also lists Husqvarna, DCA, Ingco and others), the section now opens with "Brands and products vary by stock availability. We supply products from a range of manufacturers and established suppliers…" and states that the manufacturer/brand shown on each product page reflects the product being offered. This keeps the page accurate and Merchant-Center-safe regardless of which brands are in stock. (Applies to the starter About Us page; paste the matching update into the live WordPress page from the shared About Us doc.)
- Demo import content (demo/pages.json) brought in line with the compliant live copy so a demo re-import can never reintroduce old claims. Removed "bank transfer" from Terms and FAQ payments (now M-Pesa and Pay on Delivery only), reframed the FAQ "Are your tools genuine?" question to a factual "What products do you sell?", and replaced the demo Shipping/FAQ "1-3 business days" wording with the real tiered delivery (KES 300 Nairobi / KES 500 rest of Kenya; Nairobi same/next day, major towns 1-3, remote 2-5). No live rendering change — this only affects the setup wizard's demo importer.
## 2.22.3
...
## 2.22.1
- Single product page: fixed the small product photo. WooCommerce's default layout applies float + width:48% to both the gallery and the summary; the theme's CSS grid ignores the float, but that 48% still shrank the image to half its column. The gallery is now forced to fill its column (large photo) and the summary to use its full column, with !important + body scope so the rule wins regardless of stylesheet load order.
- Shop by Category cards: images no longer go blank when a category has no image assigned in WooCommerce. Each card now falls back to the newest in-category product's photo, then to the WooCommerce placeholder image, so a card never renders an empty box. Assigning a real image in Products > Categories still takes priority. Fallback lookup is cached for 6 hours per category.
## 2.22.0
- Single product page brought up to the tabarak / frontmall layout: added a structured "product details" card above the buy button showing Brand, Model, SKU / MPN, Condition, Warranty (where applicable), Delivery (KSh 300 Nairobi · KSh 500 rest of Kenya, linked to the shipping policy) and Returns (linked to the return policy). Optional rows (Brand / Model / SKU) appear only when that data exists on the product, so nothing is invented.
- Trust badges under the buy box upgraded to a two-line title + subtitle format (Warranty where applicable / Fast delivery / Secure payment / Real support), replacing the single-line list. No "genuine / 100%" claims — Merchant-Center-safe wording throughout.
- The stand-alone single-product delivery paragraph was folded into the new details card, so delivery now shows once (concise, linked to the full tiered policy) instead of twice.
- Shipping timeline made consistent sitewide. The homepage "why choose us" and trust strip, the landing-page trust row and benefits, the single-product delivery line, and the "Why buy" grid no longer promise a flat "countrywide in 1-3 days" (which contradicted the Shipping Policy's 2-5 days for remote areas). They now read "1-5 business days", with the per-tier detail (Nairobi same/next day, major towns 1-3, remote areas 2-5) kept in the FAQ, delivery note and landing-page shipping card.
- Starter About Us page rewritten to the richer live-page structure (Our story, What we sell, Brands we carry, Why choose us, Serving Kenya, Mission & values, Visit our store) with all "Genuine / trusted source / counterfeit / warranty-backed" claims removed and the premium-brand name-drop replaced with accurate stocked-brand wording (Total, Ingco, Maxmech, DCA).
## 2.21.1
